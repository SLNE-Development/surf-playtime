...
class PlaytimeServiceImpl : PlaytimeService {
    ...
    override suspend fun loadSessions(playerUuid: UUID): ObjectSet<PlaytimeSession> =
        PaperPlaytimeInstance.rabbitApi.sendRequest(LoadSessionsByPlayerUuidRequestPacket(playerUuid)).sessions.toObjectSet()
    ...
    override suspend fun loadSessionsByServer(
        playerUuid: UUID,
        serverName: String
    ): ObjectSet<PlaytimeSession> = PaperPlaytimeInstance.rabbitApi.sendRequest(
